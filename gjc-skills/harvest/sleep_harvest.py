...
실물은 `skillopt-sleep`이 transcript를 harvest해서(~/.gjc/agent/sessions, ~/.claude/projects,
~/.codex/sessions 모두에서 찾는다) 반복 과업을 mine한 뒤 replay -> held-out gate -> adopt로 이어간다.
...
WRITERS = {"Write": "file_path", "write": "path", "Edit": "file_path", "edit": "path", "NotebookEdit": "notebook_path"}
DELEGATES = {"Task", "Agent", "subagent", "task"}
...
def logs_in_window(hours: float, all_projects: bool) -> list[str]:
    ...
    out = []
    for path in (glob.glob(os.path.expanduser("~/.gjc/agent/sessions/*/*.jsonl"))
    ...
    with open(path, encoding="utf-8") as fh:
        for raw in fh:
            ...
            for item in content:
                ...
                if item.get("type") not in ("tool_use", "toolCall"):
                    continue
                name = item.get("name", "?")
                ...
                if name in ("Skill", "SlashCommand", "skill"):
                    nm = inp.get("skill") or inp.get("command") or inp.get("name") or ""
