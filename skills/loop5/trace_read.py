...
READERS = {"Read": ("file_path",), "Grep": ("path", "glob"), "Glob": ("path",), "NotebookRead": ("notebook_path",)}
...
def find_log(explicit: str | None, since_iso: str | None = None) -> str:
    ...
    if not files:
        print("세션 로그를 찾지 못했다 (~/.claude/projects/*/*.jsonl 없음).", file=sys.stderr)
        print("Claude Code로 대화를 한 번 한 뒤 다시 실행한다.", file=sys.stderr)
        sys.exit(2)
...
def brief(name: str, inp: dict) -> str:
    for key in ("file_path", "command", "pattern", "notebook_path", "prompt", "description"):
        val = inp.get(key)
...
def main() -> None:
    ...
    if a.thinking:
        ...
        if total and not texts:
            print(f"  {RED}사고 요약이 꺼져 있다{RESET}  {DIM}(~/.claude/settings.json의 "
                  f'showThinkingSummaries: true, 켠 뒤 새 세션부터 적용){RESET}')
            return
        for t in texts:
            one = " ".join(t.split())
